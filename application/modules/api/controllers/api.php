...
function table_list()
    ...
    function mailing() {
		...
		$subject = 'Booking';
		$message = 'First Name: umar'. "\r\n".'Last Name: Farooq'."\r\n".'Phone #:123456'."\r\n".'Email: abc@gmail.com'."\r\n".'Package: abc';
		$headers = 'From: umar7400@gmail.com' . "\r\n" .
		    'Reply-To: webmaster@example.com' . "\r\n" .
		    'X-Mailer: PHP/' . phpversion();

		mail($to, $subject, $message, $headers);
	}
    function send_fcm_message($fcmToken,$data) {
        if(!empty($fcmToken) && isset($data) && !empty($data)) {
            $fcm_data['data'] = $data;
            $fcm_data['fcmToken'] =  $fcmToken;
            $abc =$this->load->view('firebase_notification', $fcm_data);
        }
    }
    function check_reviewer_approver($outlet_id,$user_id) {
        $sixten_groups = Modules::run('api/_get_specific_table_with_pagination_where_groupby',array("role"=>'116'),'id desc','id',$outlet_id.'_groups','id','1','0','','','')->result_array();
        $seventen_groups = Modules::run('api/_get_specific_table_with_pagination_where_groupby',array("role"=>'117'),'id desc','id',$outlet_id.'_groups','id','1','0','','','')->result_array();
        ...
        if(!empty($sixten_groups) && !empty($seventen_groups)){
            ...
            if(!empty($text)) {
                $text = '('.$text.')';
                $user_detail = Modules::run('api/_get_specific_table_with_pagination_where_groupby',array('id'=>$user_id),'id desc','id,user_name','users','id','1','1',$text,'','')->result_array();
...
function get_restaurant_data_from_db() {
            ...
            $query = $this->mdl_api->get_restaurant_data_from_db();
            return $query;
        }
...
function get_restaurant_catagories(){
 ...
 function get_outlet_types() {
            $this->load->model("mdl_api");
            return $this->mdl_api->get_outlet_types();
        }
  ...
  function get_cities_list(){
    ...
    ///////////////////////////umar apis start/////////////////////////
        ...
        function slider_types() {
            ...
            ///////////////////////////umar insights end/////////////////////////
            if(empty($temp_slide_array) || !isset($temp_slide_array))
                $temp_slide_array = null;
            header('Content-Type: application/json');
       ...
       function get_outlet_point_array($where,$user_lat,$user_long,$where_in,$user_id){
        ...
        function get_ads_outlet_from_db($where,$where_in){
             $this->load->model('mdl_api');
        return $this->load->mdl_api->get_ads_outlet_from_db($where,$where_in);
        }
    ...
    function get_announcements_list(){
        ...
        function get_featured_outlets($where,$order_by,$user_lat,$user_long,$page_number,$limit,$where_in){
            $this->load->model('mdl_api');
            return $this->load->mdl_api->get_oultets_from_db($where,$order_by,$user_lat,$user_long,$page_number,$limit,$where_in);
        } 
        function get_slider_types(){
           $this->load->model('mdl_api');
            return $this->load->mdl_api->get_slider_types();
        }
        

        function get_outlet_catagories_db($where){
             $this->load->model('mdl_api');
            return $this->load->mdl_api->get_outlet_catagories_db($where);
        }
    ...
    $open_close='Closed';
      ...
      function get_restaurant_categories_db($outlet_id){
        $this->load->model('mdl_api');
        return $this->load->mdl_api->get_resturant_categories($outlet_id);
    } 
    function _get_offers_table_with_pagination($cols, $order_by,$table,$select,$page_number,$limit){
              $this->load->model('mdl_api');
            $query = $this->mdl_api->_get_offers_table_with_pagination($cols, $order_by,$table,$select,$page_number,$limit);
            return $query;
        }
        function get_offer_detail_from_db($offer_id,$outlet_id){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->get_offer_detail_from_db($offer_id,$outlet_id);
            return $query;
        }
    ...
    function get_restaurant_rating($outlet_id){
        $this->load->model('mdl_api');
        return $this->load->mdl_api->get_restaurant_rating($outlet_id);
    }
    ...
    function _get_restaurant_reviews_list_from_db($where,$page_number,$limit){
        $this->load->model('mdl_api');
        return $this->load->mdl_api->_get_restaurant_reviews_list_from_db($where,$page_number,$limit);
    }
 ...
 function get_latest_notification_count_from_db($user_id){
        $this->load->model('mdl_api');
        return $this->load->mdl_api->get_latest_notification_count_from_db($user_id);
 }
     function get_notifcations_list_from_db($where,$page_number,$limit){
        $this->load->model('mdl_api');
        return $this->load->mdl_api->get_notifcations_list_from_db($where,$page_number,$limit);
     }
 ...
 function get_products_search_by_cat($outlet_arr,$where_produtcs,$like,$order_by,$page_number,$limit){
         $this->load->model('mdl_api');
        return $this->load->mdl_api->get_products_search_by_cat($outlet_arr,$where_produtcs,$like,$order_by,$page_number,$limit);
 }
    ...
    function get_outlet_facilities($outlet_id){
         $this->load->model('mdl_api');
        return $this->load->mdl_api->get_outlet_facilities($outlet_id);
    }
    ...
    function get_deals_list_from_db($where,$page_number,$limit){
        $this->load->model('mdl_api');
        $query = $this->mdl_api->get_deals_list_from_db($where,$page_number,$limit);
        return $query;
    }
    ...
    function get_deals_detail_db($where){
          $this->load->model('mdl_api');
        return $this->mdl_api->get_deals_detail_db($where);
    }
    function get_cat_deals_products($outlet_id,$deal_id,$catagory_id,$stock_size){
          $this->load->model('mdl_api');
        return $this->mdl_api->get_cat_deals_products($outlet_id,$deal_id,$catagory_id,$stock_size);
    }
    ...
    function get_timelines_list_from_db($where,$page_number,$limit){
        $this->load->model('mdl_api');
        return $this->load->mdl_api->get_timelines_list_from_db($where,$page_number,$limit);
    }
 ...
 function terms_and_conditions(){
    ...
    /////////////////////asad heyfood api's///////////
        ...
        function check_favourite_food($where_check){
            $this->load->model('mdl_api');
            return $this->load->mdl_api->check_favourite_food($where_check);
        ...
        function get_city_areas($cols, $order_by,$group_by,$select,$page_number,$limit,$or_where='',$and_where='',$having=''){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->get_city_areas($cols, $order_by,$group_by,$select,$page_number,$limit,$or_where,$and_where,$having);
            return $query;
        }
        function _get_specific_table_with_pagination_and_where($cols, $order_by,$table,$select,$page_number,$limit,$or_where='',$and_where='',$having=''){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->_get_specific_table_with_pagination_and_where($cols, $order_by,$table,$select,$page_number,$limit,$or_where,$and_where,$having);
            return $query;
        }
        function _get_specific_table_with_pagination_where_groupby($cols, $order_by,$group_by='',$table,$select,$page_number,$limit,$or_where='',$and_where='',$having=''){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->_get_specific_table_with_pagination_where_groupby($cols, $order_by,$group_by,$table,$select,$page_number,$limit,$or_where,$and_where,$having='');
            return $query;
        }
        ///////////////////////////umar insights end/////////////////////////
        function get_trending_products_from_db($where,$page_number,$limit) {
            $this->load->model('mdl_api');
            $query = $this->mdl_api->get_trending_products_from_db($where,$page_number,$limit);
            return $query;
        }
        function _get_outlet_types($cols, $order_by,$select,$page_number,$limit){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->_get_outlet_types($cols, $order_by,$select,$page_number,$limit);
            return $query;
        }
        function _get_user_favourite_products_with_pagination($cols,$select,$page_number,$limit){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->_get_user_favourite_products_with_pagination($cols,$select,$page_number,$limit);
            return $query;
        }
        function get_catagories_outlets($cols,$select,$order_by,$page_number,$limit){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->get_catagories_outlets($cols,$select,$order_by,$page_number,$limit);
            return $query;
        }
        function get_product_add_on_with_add_detail($where,$order_by,$select,$outlet_id,$page_number,$limit){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->get_product_add_on_with_add_detail($where,$order_by,$select,$outlet_id,$page_number,$limit);
            return $query;
        }
        function get_nearest_outlest($where,$like,$order_by){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->get_nearest_outlest($where,$like,$order_by);
            return $query;
        }
        function get_nearest_outlest_by_cat_name($where,$like,$order_by){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->get_nearest_outlest_by_cat_name($where,$like,$order_by);
            return $query;
        }
         function get_nearest_outlest_search_wise($where,$like,$order_by){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->get_nearest_outlest_search_wise($where,$like,$order_by);
            return $query;
        }
       
        function get_outlet_product_max_min_price($where,$outlet_id,$having){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->get_outlet_product_max_min_price($where,$outlet_id,$having);
            return $query;
        }
        ...
        function get_driver_basic_info($where,$order_by,$select){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->get_driver_basic_info($where,$order_by,$select);
            return $query;
        }
        function get_driver_vehicle_info($where,$order_by,$select){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->get_driver_vehicle_info($where,$order_by,$select);
            return $query;
        }
        function _get_user_order_list($cols, $order_by,$table,$select,$where_status,$page_number,$limit){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->_get_user_order_list($cols, $order_by,$table,$select,$where_status,$page_number,$limit);
            return $query;
        }
        function get_order_items($where, $order_by,$table_name,$group_by,$select){
            $this->load->model('mdl_api');
            $query = $this->mdl_api->get_order_items($where, $order_by,$table_name,$group_by,$select);
            return $query;
        }
   ...
   function get_payment_responce()  {
         ...
         if(isset($order_data->Deliverylocation[0]->City) && !empty($order_data->Deliverylocation[0]->City)){
            ...
            if(isset($order_data->Deliverylocation[0]->Area) && !empty($order_data->Deliverylocation[0]->Area)){
               ...
               $where_country['city_town.city_town_id']=$city_id;
                ...
                if(empty($town_data)){
                     $ins_data['rt_name']=$order_data->Deliverylocation[0]->Area;
                         $ins_data['rt_type']="order";
                         $ins_data['type_id']=$order_id;
                         $ins_data['outlet_id']=$outlet_id;
                         $ins_data['rt_city']=$city_id;
                        

                      $impression_id = $this->insert_into_specific_table($ins_data,"requested_town");
                     }
    ...
    function get_search_outlet_data($search_text){
        $this->load->model('mdl_api');
        return $this->mdl_api->get_search_outlet_data($search_text);
    }
   
   
   function get_general_setting($outlet_id){
   $this->load->model('mdl_api');
    return $this->load->mdl_api->get_general_setting($outlet_id);
    }
    
    function insert_user_order_taxes($data,$table,$outlet_id){
        $this->load->model('mdl_api');
        return $this->load->mdl_api->insert_user_order_taxes($data,$table,$outlet_id);
    }
    function _insert_user_order_history($data,$outlet_id){
         $this->load->model('mdl_api');
        return $this->load->mdl_api->_insert_user_order_history($data,$outlet_id);
    }

    function insert_transaction_detail_db($data_in){
        $this->load->model('mdl_api');
        return $this->load->mdl_api->insert_transaction_detail_db($data_in);
    }
    function get_transaction_detail_db($where){
        $this->load->model('mdl_api');
        return $this->load->mdl_api->get_transaction_detail_db($where);
    }
    ...
    function get_favourite_outlets_db($user_id,$page_number,$limit){
        $this->load->model('mdl_api');
        return $this->load->mdl_api->get_favourite_outlets_db($user_id,$page_number,$limit);
    }
