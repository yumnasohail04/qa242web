...
<div class="content-wrapper">
    ...
    <h3><?php if($this->uri->segment(3) == 'truck_inspection') echo "Truck"; elseif($this->uri->segment(3) == 'overdue_checks') echo "Overdue"; elseif($this->uri->segment(3) == 'pending_review') echo "Pending";  elseif($this->uri->segment(3) == 'pending_approval') echo "Pending Approval"; else echo ""; ?> Inspection</h3>
                        ...
                        <table class="table table-striped table-hover table-body table-bordered">
                            <thead class="bg-th">
                                <tr class="bg-col">
                                    ...
                                    <th class="text-center"style="width:200px;" >Invoice No.<i class="fa fa-sort" style="font-size:13px;"></th>
                            ...
                            <tbody id="ajax_content_wrapper">
                                ...
                                if (isset($inspection)) {
                                    foreach ($inspection->result() as
                                        ...
                                        $i++;
                                        ?>
                                    <tr  id="Row_<?=$new->ti_id?>" class="odd gradeX "  >
...
$(document).ready(function(){
    ...
    /*//////////////////////// code for detail //////////////////////////*/
            ...
            var id = $(this).attr('rel');
              $.ajax({
                        type: 'POST',
                        url: "<?= ADMIN_BASE_URL?>assignments/truck_inspection_detail",
            ...
            function submit_status() {
                $('.status_button').off('click').click(function(event){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "<?= ADMIN_BASE_URL?>assignments/truck_inspection_status",
                        data: {'id': id,'txt':txt},
                        async: false,
                        success: function(test_body) {
                        	location.reload();
...
<script>
    ...
    function ajax_call(page_number,startdate,enddate) {
        citycurrentRequest= $.ajax({
            type: "POST",  
            url: '<?= ADMIN_BASE_URL?>assignments/get_truck_search',  
