                        ...
                        <table class="table table-striped table-hover table-body table-bordered">
                            <thead class="bg-th">
                                <tr class="bg-col">
                                    ...
                                    <th class="text-center"style="width:200px;" >Circle<i class="fa fa-sort" style="font-size:13px;"></th>
                            ...
                            <tbody  id="ajax_content_wrapper">
                                ...
                                if (isset($inspection)) {
                                    foreach ($inspection->result() as
                                        ...
                                        $i++;
                                        ?>
                                    <tr  id="Row_<?=$new->ci_id?>" class="odd gradeX "  >
                                        <td class="text-center">
                                            <?php echo date('m-d-Y',strtotime($new->ci_datetime)); ?>
                                        </td>
                                        <td class="text-center">
                                            <?php echo date('H:i:s',strtotime($new->ci_datetime)); ?>
                                        </td>
                                        <td class="text-center">
                                            <?php 
                                                if(isset($new->ci_monitor_name) && !empty($new->ci_monitor_name))
                                                $review_user_detail = Modules::run('api/_get_specific_table_with_pagination',array("id"=>$new->ci_monitor_name),'id desc','users','first_name,last_name','1','1')->result_array();
                                                $fisrt_name=''; 
                                        ...
                                        <td class="text-center">
                                            <?php echo $new->ci_circle;?>
                                        </td>
                                        <td class="text-center">
                                            <?php echo $new->ci_status;?>
                                        </td>
                                        <td>
                                            <a class="btn yellow c-btn view_details" rel="<?=$new->ci_id?>"><i class="fa fa-list"  title="See Detail"></i></a>
                                        </td>
                                    </tr>
                                    <?php } ?>    
                                <?php } ?>
...
$(document).ready(function(){
    ...
    /*//////////////////////// code for detail //////////////////////////*/
            ...
            var id = $(this).attr('rel');
              $.ajax({
                        type: 'POST',
                        url: "<?= ADMIN_BASE_URL?>assignments/cleaning_inspection_detail",
            ...
            function submit_status() {
                $('.status_button').off('click').click(function(event){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "<?= ADMIN_BASE_URL?>assignments/cleaning_inspection_status",
                        data: {'id': id,'txt':txt},
                        async: false,
                        success: function(test_body) {
                        }
...
<script>
    ...
    function ajax_call(page_number,startdate,enddate) {
        citycurrentRequest= $.ajax({
            type: "POST",  
            url: '<?= ADMIN_BASE_URL?>assignments/get_cleaning_search',  
