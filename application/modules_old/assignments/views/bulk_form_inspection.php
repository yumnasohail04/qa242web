                        ...
                        <table class="table table-striped table-hover table-body table-bordered">
                            <thead class="bg-th">
                                <tr class="bg-col">
                                    ...
                                    <th class="text-center"style="width:200px;" >Packing Item<i class="fa fa-sort" style="font-size:13px;"></th>
                            ...
                            <tbody  id="ajax_content_wrapper">
                                ...
                                if (isset($inspection)) {
                                    foreach ($inspection->result() as
                                        ...
                                        $i++;
                                        ?>
                                    <tr  id="Row_<?=$new->bfi_id?>" class="odd gradeX "  >
...
$(document).ready(function(){
    ...
    /*//////////////////////// code for detail //////////////////////////*/
            ...
            var id = $(this).attr('rel');
              $.ajax({
                        type: 'POST',
                        url: "<?= ADMIN_BASE_URL?>assignments/bulk_form_inspection_detail",
            ...
            function submit_status() {
                $('.status_button').off('click').click(function(event){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "<?= ADMIN_BASE_URL?>assignments/bulk_form_inspection_status",
                        data: {'id': id,'txt':txt},
                        async: false,
                        success: function(test_body) {
                        location.reload();
...
<script>
    ...
    function ajax_call(page_number,startdate,enddate) {
        citycurrentRequest= $.ajax({
            type: "POST",  
            url: '<?= ADMIN_BASE_URL?>assignments/get_bulk_form_search',  
