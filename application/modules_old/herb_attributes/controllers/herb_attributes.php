...
class Herb_attributes extends MX_Controller {
...
function _insert_attributes_data($data){
        $this->load->model('mdl_herb_attributes');
       return $this->mdl_herb_attributes->_insert_attributes_data($data);
    }
    function get_discounts_arr_from_db($update_id){
         $this->load->model('mdl_herb_attributes');
       return $this->mdl_herb_attributes->get_discounts_arr_from_db($update_id);
    }
    function delete_org_outlet_db($arr_col){
        $this->load->model('mdl_herb_attributes');
        $this->mdl_herb_attributes->delete_org_outlet_db($arr_col);
    }
...
redirect(ADMIN_BASE_URL . 'herb_attributes/manage/'.$ccheckid.'/');

}

function _get_sub_catagories_attibutes($where,$order_by){
      $this->load->model('mdl_herb_attributes');
       return $this->mdl_herb_attributes->_get_sub_catagories_attibutes($where,$order_by);
}
...
function _update_catagories_attributes($where,$data){
    $this->load->model('mdl_herb_attributes');
       return $this->mdl_herb_attributes->_update_catagories_attributes($where,$data);
...
function _delete_attributes($where){
    $this->load->model('mdl_herb_attributes');
       return $this->mdl_herb_attributes->_delete_attributes($where);
