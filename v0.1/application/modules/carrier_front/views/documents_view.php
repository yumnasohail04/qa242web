        ...
        <?php 
            ...
            <tr data-id="<?php echo $values['id']; ?>">
                ...
                <?php if(!empty($values['doc_uploaded'])){ ?>
                    <a><img class="pdf-img" src="<?php echo STATIC_FRONT_IMAGE.'doc.png' ?>"  rel-exist="1"></a><br>
                    ...
                    <?php endforeach; ?>
                <?php }else{ ?>
                    <img src="<?php echo STATIC_FRONT_IMAGE.'Delete-file-icon.png' ?>" class="pdf-img"  rel-exist="0">
                <?php  
                } ?>
                </td>
                <?php 
                if(!empty($values['sub_question'])){
                    if($values['sub_question']->comment_box=="1"){?>
                        <td ><input type="text" name="comment_<?php echo $keys; ?>" class="form-control comment_opt" placeholder="Comments Here..." value="<?php if(isset($values['sub_ans']->comment_box) && !empty($values['sub_ans']->comment_box)) echo $values['sub_ans']->comment_box;?>" c_exist="<?php if(!empty($values['sub_ans']->comment_box)) echo "1"; else "0";?>"></td>
                    <?php }else{ ?>
                        <td></td>
                    <?php }
                    }else{ ?>
