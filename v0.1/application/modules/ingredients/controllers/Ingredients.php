    ...
    function get_form_data()
    {
        ...
        $data['question'] = Modules::run('api/_get_specific_table_with_pagination_where_groupby',array("status" =>'1','del_status'=>'0'),'id asc','id','ing_form_questions','*','1','0','','','')->result_array();
			foreach($data['question'] as $key => $value):
				...
				$ans=array();
				$ans = Modules::run('api/_get_specific_table_with_pagination_where_groupby',array("quest_id" =>$value['id'],"ing_id"=>$ing_id,"supplier_id"=>$supplier,"submit"=>"1"),'id desc','id',' ing_form_ans','comment,document,expiry,id','1','0','','','')->result_array();
    ...
    function submit_ing_form_data()
		{
			...
			redirect(ADMIN_BASE_URL.'ingredients/create/'.$ing_id);
    		exit();
		}
