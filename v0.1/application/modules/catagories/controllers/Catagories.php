...
function delete_org_outlet_db($arr_col){
    ...
    function submit_sub_catagories_attributes() {
        ...
        if ($update_id && $update_id != 0) {
            ...
            if(!empty($data['attribute_type'])) {
                if(strtolower($data['attribute_type']) == 'choice') {
                    if(strtolower($data['possible_answers']) == 'other')  {
                        $choice_name = $this->input->post('choice_name');
                        $acceptance = $this->input->post('acceptance');
                        $others = $this->input->post('others');
                        $previous_others = Modules::run('api/_get_specific_table_with_pagination_where_groupby',array('opt_question_id'=>$update_id),'id desc','id','attribute_other_options','id','1','0','','','')->result_array();
                        if(!empty($previous_others)) {
                            foreach ($previous_others as $po_key => $po):
                                if(empty(in_array($po['id'], $others)))
                                    Modules::run('api/update_specific_table',array("id"=>$po['id']),array("opt_delete"=>"1"),'attribute_other_options');
                            endforeach;
                        }
                        if(!empty($choice_name)) {
                            foreach ($choice_name as $cn_key => $cn):
                                ...
                                if(!empty($others[$cn_key]))
                                    $checking = array_search($others[$cn_key], array_column($previous_others, 'id'));
                                if(!is_numeric($checking)) {
                                    Modules::run('api/insert_into_specific_table',array("opt_question_id"=>$update_id,"opt_option"=>$cn,"opt_acceptance"=>$acceptance[$cn_key],"opt_status"=>"1"),'attribute_other_options');
                                }
                                elseif(is_numeric($checking)) {
                                    Modules::run('api/update_specific_table',array("id"=>$others[$cn_key]),array("opt_question_id"=>$update_id,"opt_option"=>$cn,"opt_acceptance"=>$acceptance[$cn_key],"opt_status"=>"1"),'attribute_other_options');
                                }
                            endforeach;
                        }
                    }
                    ...
                    if(strtolower($data['possible_answers']) == 'other')  {
                        ...
                        if(!empty($choice_name)) {
                            foreach ($choice_name as $keying => $cn):
                                Modules::run('api/insert_into_specific_table',array("opt_question_id"=>$id,"opt_option"=>$cn,"opt_acceptance"=>$acceptance[$keying]),'attribute_other_options');
                            endforeach;
                        }
                    }
                }
    ...
    function _get_data_from_post_attributes($check_cat_id){
        ...
        $data['selection_type']=$this->input->post('selection_type');
        if(strtolower($data['attribute_type']) == 'choice' && empty($data['selection_type']))
        $data['selection_type']="single_select";
    ...
    function _get_data_from_db_attributes($attribute_id,$check_cat_id){
        ...
        foreach ($result->result() as $key => $value) {
           ...
           $data['selection_type']=$value->selection_type;
        }
        return $data;
    }
