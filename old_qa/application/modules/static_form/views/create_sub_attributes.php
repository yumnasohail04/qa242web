          ...
          <?php
            ...
            else
            {
               $strTitle = "New Attribute Detail";
                    ...
                    echo $strTitle;
                    ?>
                    <a href="<?php echo ADMIN_BASE_URL.'static_form/submit_attributes/'.$parent_id.'/'.$cat_name; ?>"><button type="button" class="btn btn-primary pull-right"><i class="fa fa-chevron-left"></i>&nbsp;&nbsp;&nbsp;Back</button></a>
    </h3>
    </div>
    <div class="row">
      <div class="col-md-12">
                                      ...
                                      <div>
                                        ...
                                        <?php }
                                          if(!empty($attribute_type) && $attribute_type=="range") {
                                            $ref_value = Modules::run('api/_get_specific_table_with_pagination_where_groupby',array('sfa_delete'=>'0','sfa_question_id'=>$update_id,'lower(sfa_answer_acceptance)'=>'refrigerated'), 'sfa_id desc', 'sfa_id',DEFAULT_OUTLET.'_static_form_answer','sfa_id,sfa_min,sfa_max,sfa_target,sfa_answer_acceptance','1','1','','','')->row_array();
                                            $ref_id = ""; 
                                            if(isset($ref_value['sfa_id']) && !empty($ref_value['sfa_id']))
                                              $ref_id = $ref_value['sfa_id'];
                                            ...
                                            ?>
                                              <div class="col-md-5">
                                                <div class="form-group">
                                                  ...
                                                  <div class="col-md-8">
                                                    <input type="text" name="refrigerated_min_value" value="<?php $text = '';
                                                    if(isset($ref_value['sfa_min']) && !empty($ref_value['sfa_min']))
                                                        $text = $ref_value['sfa_min'];
                                                        echo $text;
                                                    ?>" id="refrigerated_min_value" class="form-control validate[required] text-input " required="required">
                                                  </div>
                                                </div>
                                              </div>
                                              <div class="col-md-6">
                                                <div class="form-group">
                                                  ...
                                                  <div class="col-md-8">
                                                    <input type="text" name="refrigerated_max_value" value="<?php $text = '';
                                                    if(isset($ref_value['sfa_max']) && !empty($ref_value['sfa_max']))
                                                        $text = $ref_value['sfa_max'];
                                                        echo $text;
                                                    ?>" id="refrigerated_max_value" class="form-control validate[required] text-input " required="required">
                                                  </div>
                                                </div>
                                              </div>
                                              <div class="col-md-5 marging">
                                                <div class="form-group">
                                                  ...
                                                  <div class="col-md-8">
                                                    <input type="text" name="refrigerated_target_value" value="<?php $text = '';
                                                    if(isset($ref_value['sfa_target']) && !empty($ref_value['sfa_target']))
                                                        $text = $ref_value['sfa_target'];
                                                        echo $text;
                                                    ?>" id="possible_answer" class="form-control validate[required] text-input " required="required">
                                                  </div>
                                                </div>
                                              </div>
                                              <div class="col-sm-6 frozing marging" style="float: right;">
                                                <div class="form-group">
                                                  <label for="lstRank" class="control-label  col-md-7" style="float:right;margin-left: 20px;">
                                                    <button class="btn btn-info clone-frozen">Add Frozen Temperature</button>
                                                  </label>
                                                </div>
                                              </div>
                                            <?php unset($ref_value); $ref_value = Modules::run('api/_get_specific_table_with_pagination_where_groupby',array('sfa_delete'=>'0','sfa_question_id'=>$update_id,'lower(sfa_answer_acceptance)'=>'frozen'), 'sfa_id desc', 'sfa_id',DEFAULT_OUTLET.'_static_form_answer','sfa_id,sfa_min,sfa_max,sfa_target,sfa_answer_acceptance','1','1','','','')->row_array();
                                            $check = false;
                                            if(isset($ref_value['sfa_min']) && !empty($ref_value['sfa_min']))
                                              $check = true;
                                            elseif(isset($ref_value['sfa_max']) && !empty($ref_value['sfa_max']))
                                              $check = true;
                                            elseif(isset($ref_value['sfa_target']) && !empty($ref_value['sfa_target']))
                                              $check = true;
                                            else
                                              $check = false;
                                            if( $check == true) {
                                            $frozen_id = ""; 
                                            if(isset($ref_value['sfa_id']) && !empty($ref_value['sfa_id']))
                                              $frozen_id = $ref_value['sfa_id'];
                                            echo "<input type='hidden' name='frozen_id' value='".$frozen_id."'>"; ?>
                                              <span class="marging frozensec col-md-12">
                                                <span class="removedealsitem_frozen">x</span>
                                                <div class="col-md-5">
                                                  <div class="form-group">
                                                    <label for="txtCatName" class="control-label col-md-4">
                                                      Frozen Min Value
                                                    ...
                                                    <div class="col-md-8">
                                                      <input type="text" name="frozen_min_value" value="<?php $text = '';
                                                    if(isset($ref_value['sfa_min']) && !empty($ref_value['sfa_min']))
                                                        $text = $ref_value['sfa_min'];
                                                        echo $text;
                                                    ?>" id="refrigerated_min_value" class="form-control validate[required] text-input " required="required">
                                                ...
                                                <div class="col-md-6">
                                                  <div class="form-group">
                                                    ...
                                                    <div class="col-md-8">
                                                      <input type="text" name="frozen_max_value" value="<?php $text = '';
                                                    if(isset($ref_value['sfa_max']) && !empty($ref_value['sfa_max']))
                                                        $text = $ref_value['sfa_max'];
                                                        echo $text;
                                                    ?>" id="frozen_max_value" class="form-control validate[required] text-input " required="required">
                                                    </div>
                                                  </div>
                                                </div>
                                                <div class="col-md-5 marging">
                                                  <div class="form-group">
                                                    ...
                                                    <div class="col-md-8">
                                                      <input type="text" name="frozen_target_value" value="<?php $text = '';
                                                    if(isset($ref_value['sfa_target']) && !empty($ref_value['sfa_target']))
                                                        $text = $ref_value['sfa_target'];
                                                        echo $text;
                                                    ?>" id="frozen_target_value" class="form-control validate[required] text-input " required="required">
                                                    </div>
                                                  </div>
                                                </div>
                                              </span>
                                          ...
                                          if(isset($sfq_selection_type) && !empty($sfq_selection_type)) {
                                            if(strtolower($sfq_selection_type) == 'other') { 
                                              if(!empty($this->uri->segment(7)) && is_numeric($this->uri->segment(7))) {
                                                $answers = Modules::run('api/_get_specific_table_with_pagination',array("sfa_question_id" =>$this->uri->segment(7)),'sfa_id asc',DEFAULT_OUTLET.'_static_form_answer','sfa_id,sfa_answer,sfa_answer_acceptance','1','0')->result_array();
                                                if(!empty($answers)) {
                                                  $counter = 1;
                    ...
                    <div class="col-md-6">
                      <div class="col-md-offset-2 col-md-9" style="padding-bottom:15px;">
                       <span style="margin-left:40px"></span> <button type="submit" class="btn btn-primary"><i class="fa fa-check"></i>&nbsp;Save</button>
                        <a href="<?php echo ADMIN_BASE_URL . 'static_form/manage_attributes/'.$parent_id.'/'.$check_name; ?>">
