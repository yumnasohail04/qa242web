...
class ControllerList {
	...
	private function setControllers() {
		$this->CI->config->load('admin_left_nav');
		$modules = $this->CI->config->item('modules');
		//$modules = 	  $tables = $this->CI->db->list_tables();
		//echo'<pre>'; print_r($modules).'<pre><hr>';
		foreach($modules as $module){
		// Loop through the controller directory
		//echo'<pre>'; print_r(glob(APPPATH . "modules/$module/controllers/*")); echo'</pre><br>conroller==>>'.$controller.'<hr>';
		if(glob(APPPATH . "modules/$module/controllers/*") != ''){
			...
			if(is_dir($controller)) {
				...
				foreach(glob(APPPATH . "controllers/modules/$module/".$dirname."/*") as $subdircontroller) {
